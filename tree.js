...

{
  ...
  class SimpleTree extends Emitter {
   ...
   isFolder(el){
	 if (el.node)
	    return (el.node.type === SimpleTree.FOLDER);
   }
   removeItem(){
	var el = this.active();
	if (el){
		if (this.isFolder(el)){
			this.unloadFolder(el);
   ...
   sumNodeChilds(e){
        if (e.childNodes[0].tagName == 'SUMMARY'){
            if (!e.childNodes[0].node)
                return false;
			if (!e.childNodes[0].node.children)
				e.childNodes[0].node.children = new Array;
			var s = e.childNodes[0].node.children;
		}else{
			if (!e.parentElement.childNodes[0].node.children)
				e.parentElement.childNodes[0].node.children = new Array;
			var s = e.parentElement.childNodes[0].node.children;
		}
	   return s;
   }
   updtNode(n, p, it){
   ...
   addNode(aNode, pos, it){
	var oFolder = this.sumNodeChilds(aNode);
	if (oFolder){
	if (this.isFolder(aNode.childNodes[0]))
		aNode.childNodes[0].node.children = this.updtNode(oFolder, pos-1, it);
	else
		aNode.parentElement.childNodes[0].node.children = this.updtNode(oFolder, pos-1, it);
	}
   }
   removeNode(aNode){
	var tNode = aNode;
	if (this.isFolder(aNode)){
	...
	var oFolder = this.sumNodeChilds(aNode);
	if (oFolder){
	var pos = Array.from(aNode.children).indexOf(tNode);
	var n = this.updtNode(oFolder, (pos * -1));
	aNode.childNodes[0].node.children = n;
	if (n[0] == null)
		delete aNode.childNodes[0].node.children;
	}
   }
  }
  ...
  class SelectTree extends AsyncTree {
    constructor(parent, options = {}) {
      ...
      parent.addEventListener('focus', () => {	//window
        const active = this.active();
        if (active) {
          this.focus(active);
        }
    ...
    navigate(direction = 'forward', noSelect) {
      const e = this.active();
      if (e) {
